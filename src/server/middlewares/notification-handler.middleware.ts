...
const notificationMapping: Record<string, NotificationConfigInterface> = {
  'customer.update': {
    roles: ['bank-owner', 'bank-manager'],
    key: 'customer-account-update',
    tenantPath: ['bank', 'user', 'customer'],
    userPath: [],
  },
  'customer.delete': {
    roles: ['bank-owner', 'bank-manager'],
    key: 'customer-account-deletion',
    tenantPath: ['bank', 'user', 'customer'],
    userPath: [],
  },
  'transaction.create': { roles: ['bank-owner', 'bank-manager'], key: 'new-transaction', tenantPath: [], userPath: [] },
};

const ownerRoles: string[] = ['bank-manager'];
const customerRoles: string[] = ['customer'];
const tenantRoles: string[] = ['bank-owner', 'bank-employee', 'bank-manager', 'customers', 'accounts'];
